        ...
        .then((data) => {
            if (data["status"] === "Success") {
                    ...
                    .then((dataCards) => {
                        ...
                        if (dataCards.hasCard) {
                            ...
                            document.getElementById(
                                "card-last4"
                            ).textContent = `**** **** **** ${dataCards.cardDetails.last4}`;
                            document.getElementById(
                                "card-expiry"
                            ).textContent = `${dataCards.cardDetails.exp_month}/${dataCards.cardDetails.exp_year}`;
                            document.getElementById("customerId").value =
                                dataCards.customerId;
                            document.getElementById("paymentMethodId").value =
                                dataCards.paymentMethodId;
                        } else {
                ...
                modalContent.innerHTML = `<p>${
                    data.message || "Error fetching borrower details."
                }</p>`;
            }
        })
        .catch(console.error);
...
document.addEventListener("DOMContentLoaded", () => {
    const stripe = Stripe(
        "pk_test_51O7q8CJvFHmzlX92OVAQU6H0GFuN5tiUGEdOy4bNFa4hWbTWEPFA4YFMQl1Pye6FkFkl0npuYAUyPZFmMzgzau6o00uSYykKHk"
    );
...
async function handleCardSubmission(stripe, elements) {
    ...
    try {
        ...
        if (error) {
            ...
            await saveCardDetails(email, paymentMethod.id);
        }
    } catch (error) {
...
async function saveCardDetails(email, paymentMethodId) {
    try {
        const response = await fetch("http://localhost:8000/payment/card/add", {
            ...
            body: JSON.stringify({ email, paymentMethodId }), // Removed userId
        });
        ...
        if (data.cardDetails) {
            ...
            document.getElementById(
                "card-last4"
            ).textContent = `**** **** **** ${cardDetails.last4}`;
            document.getElementById(
                "card-expiry"
            ).textContent = `${cardDetails.exp_month}/${cardDetails.exp_year}`;
